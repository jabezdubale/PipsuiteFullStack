...
function App() {
  ...
  useEffect(() => {
    const initData = async () => {
      // 1. Theme, User, & Date Prefs (Sync from DB)
      const [themePref, userProfile, datePref] = await Promise.all([
          getSetting<string>('pipsuite_theme', 'light'),
  ...
  const handleDateRangeChange = (start: string, end: string) => {
      setStartDate(start);
      setEndDate(end);
      saveSetting('pipsuite_date_range', { start, end });
  };
  ...
  const stats = useMemo(() => {
      // Calculate stats based on current filtered view (by date) for Dashboard consistency
      // Filter trades by date range for stats calculation
      const filteredForStats = currentAccountTrades.filter(t => {
      ...
      const wins = filteredForStats.filter(t => t.pnl > 0);
      const losses = filteredForStats.filter(t => t.pnl < 0);
      const totalTrades = filteredForStats.length;
      
      const netPnL = filteredForStats.reduce((acc, t) => acc + t.pnl, 0);
      ...
      return {
          ...
          profitFactor,
          bestTrade: Math.max(...filteredForStats.map(t => t.pnl), 0),
          worstTrade: Math.min(...filteredForStats.map(t => t.pnl), 0)
      };
  }, [currentAccountTrades, startDate, endDate]);
  ...
  const handleImportTrades = async (importedTrades: Trade[]) => {
      ...
      // Auto-expand date range to include all new trades
      if (importedTrades.length > 0) {
          const timestamps = importedTrades.map(t => new Date(t.entryDate || t.createdAt).getTime()).filter(t => !isNaN(t));
