...
import { getTrades, saveTrade, deleteTrades, getAccounts, saveAccount, deleteAccount, getTagGroups, saveTagGroups, getStrategies, saveStrategies, saveTrades } from './services/storageService';
...
import { Trade, TradeStats, Account, TradeType, TradeStatus, ASSETS, TagGroup, OrderType, Session, TradeOutcome } from './types';
...
function App() {
  ...
  useEffect(() => {
    if (isAddModalOpen && selectedAccountId) {
      ...
      if (acc) {
        setNewTradeForm(prev => ({ ...prev, balance: acc.balance, symbol: prev.symbol || 'XAUUSD' }));
      }
    }
