...
const TradeDetail: React.FC<TradeDetailProps> = ({ trade, accounts, tagGroups, strategies, onSave, onDelete, onBack, onUpdateBalance }) => {
  ...
  const [formData, setFormData] = useState<any>({
    ...
    entryTime: trade.entryTime || '', // User entered time string
    exitTime: trade.exitTime || '', // User entered time string
    // Main Profit defaults to empty string if undefined (so it shows placeholder instead of 0)
  ...
  // Fixed: Replaced NodeJS.Timeout with ReturnType<typeof setTimeout> for compatibility
  ...
  const calculatedFinancials = useMemo(() => {
    ...
    // Check if mainPnl is effectively set (not empty string, not null)
    ...
    // Net P&L Display Logic
    let netPnlDisplay: string | number = '-';
    let netPnlValue = 0; // Value used for internal storage/logic

    if (hasMainPnl) {
        // If Main Profit is added, Net P&L = Main + Partials
        netPnlValue = mainPnlVal + partialsTotal;
        netPnlDisplay = netPnlValue;
    } else if (hasPartials) {
        // If Partials added but no Main Profit, Net P&L = Partials (as intermediate calc)
    ...
    // Fees Logic: Fees = Planned Reward - Net P&L
    // Display only if Main Profit is present (implies trade is fully accounted for)
  ...
  const performSave = (currentFormData: any, currentFinancials: any) => {
      ...
      // --- CALCULATE AUTO TAGS ---
      // We apply this on every save to keep tags in sync with partials/prices changes
      ...
      const updatedTrade: Trade = {
          ...currentFormData,
          tags: updatedTags, // Apply calculated tags
          entryPrice: parseFloat(currentFormData.entryPrice),
          exitPrice: currentFormData.exitPrice ? parseFloat(currentFormData.exitPrice) : undefined,
          quantity: parseFloat(currentFormData.quantity),
          // Save fees as number if available, otherwise 0
          fees: typeof currentFinancials.feesDisplay === 'number' ? currentFinancials.feesDisplay : 0, 
          takeProfit: currentFormData.takeProfit ? parseFloat(currentFormData.takeProfit) : undefined,
          stopLoss: currentFormData.stopLoss ? parseFloat(currentFormData.stopLoss) : undefined,
          // Save mainPnl as undefined if empty string, to allow UI to show placeholder next time
          mainPnl: currentFormData.mainPnl === '' ? undefined : parseFloat(currentFormData.mainPnl),
          pnl: net,
          status,
          // Ensure isBalanceUpdated persists correctly from formData (which might have been updated above)
          isBalanceUpdated: currentFormData.isBalanceUpdated
      };
      
      // Update local tags state if they changed so the UI reflects it immediately
      ...
      onSave(updatedTrade, false); // false = do not close
  };
  ...
  const handleConfirmReopen = () => {
      // Revert Balance Logic if previously closed and affected balance
      if (formData.isBalanceUpdated && onUpdateBalance) {
          const previousPnl = formData.pnl; // The PnL that was applied
          // If PnL was positive (profit), we added to balance, so now we Withdraw.
          // If PnL was negative (loss), we deducted, so now we Deposit.
  ...
  const handleConfirmMissed = () => {
      // Revert Balance Logic if coming from CLOSED
      ...
      // Reset Financials (Core Profit & Partials)
      const updatedForm = {
          ...formData,
      ...
      // Force Save with cleared values
      const tradeToSave: Trade = {
          ...updatedForm,
          entryPrice: parseFloat(updatedForm.entryPrice),
          exitPrice: updatedForm.exitPrice ? parseFloat(updatedForm.exitPrice) : undefined,
  ...
  const handleCloseModalConfirm = (closedData: any) => {
      // 1. Prepare data (without affecting balance flag as it's not part of Trade type)
      ...
      // Remove the temp flag before saving
      const affectBalance = closedData.affectBalance;
      delete updatedFormData.affectBalance;

      // 2. Calculate final stats for the trade to be saved
      ...
      // **CRITICAL FIX**: Update local form state PnL so if we reopen immediately, we revert the correct amount
      updatedFormData.pnl = net;

      // --- CALCULATE AUTO TAGS (Close Modal Confirm) ---
      const updatedTags = calculateAutoTags({
      ...
      // 4. Update local state and Save trade immediately
      setFormData(updatedFormData);
      
      // Ensure we call with shouldClose = false
      onSave(finalTradeToSave, false);
      setIsCloseModalOpen(false);
  };

  const handleDateTimeChange = (field: 'entry' | 'exit', value: string) => {
      // value comes from input type="datetime-local" (YYYY-MM-DDTHH:mm) or empty
      if (!value) {
          // User cleared the input
          setFormData((prev: any) => ({
             ...prev,
             [`${field}Time`]: '', // Clear execution time flag
             [`${field}Session`]: Session.NONE,
             // If exit, clear date too. If entry, keep date (as Log Date).
  ...
  const handleRemovePartial = (id: string) => {
    setFormData((prev: any) => ({
        ...prev,
        partials: prev.partials.filter((p: TradePartial) => p.id !== id)
    }));
  };
  ...
  // ... rest of the file ...
  // (No other changes needed in render, keeping existing JSX)
  // ...
