...
const AddTradeModal = ({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
      e.preventDefault();
      console.log("DEBUG: AddTradeModal handleSubmit triggered. Current FormData:", formData);

      if (!formData.entryPrice || !formData.quantity || !formData.accountId) {
          console.warn("DEBUG: AddTradeModal validation failed. Missing EntryPrice, Quantity or AccountId.");
      ...
      console.log("DEBUG: AddTradeModal sending newTrade to onSave:", newTrade);

      try {
          const success = await onSave(newTrade);
          console.log("DEBUG: AddTradeModal onSave result:", success);
          if (success) {
              setFormData({
          ...
          console.error("DEBUG: AddTradeModal onSave exception:", err);
      } finally {
...
function App() {
  ...
  const handleSaveTrade = async (trade: Trade, shouldClose: boolean = true): Promise<boolean> => {
    console.log("DEBUG: App.handleSaveTrade called with:", trade);
    try {
        await saveTrade(trade);
        console.log("DEBUG: App.handleSaveTrade service call successful");
        // Refresh trades to get updated list (including ID if generated by DB, though we gen ID client side currently)
        ...
        console.error("DEBUG: App.handleSaveTrade failed:", e);
